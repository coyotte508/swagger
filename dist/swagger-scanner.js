...
class SwaggerScanner {
    ...
    scanApplication(app, options) {
        const { deepScanRoutes, include: includedModules = [], extraModels = [], ignoreGlobalPrefix = false, operationIdFactory, linkNameFactory } = options;
        ...
        const denormalizedPaths = modules.map(({ routes, metatype, relatedModules }) => {
            ...
            const path = metatype
                ...
                : undefined;
            return this.scanModuleRoutes(allRoutes, path, globalPrefix, operationIdFactory, linkNameFactory);
        });
    ...
    scanModuleRoutes(routes, modulePath, globalPrefix, operationIdFactory, linkNameFactory) {
        const denormalizedArray = [...routes.values()].map((ctrl) => this.explorer.exploreController(ctrl, modulePath, globalPrefix, operationIdFactory, linkNameFactory));
